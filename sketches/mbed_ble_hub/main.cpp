...
void advertisementCallback(const Gap::AdvertisementCallbackParams_t *params) {
    ...
    bool isSensorMine = false;
    size_t size = sizeof(home_sensors_address) / sizeof(BLEProtocol::AddressBytes_t);
    for (int i=0;i<size;i++) {
        
        if (0 == memcmp(params->peerAddr, home_sensors_address[i], sizeof (home_sensors_address[i]))) {           
           isSensorMine = true;
           break;
        }
    }    
