...
*                        20190831 - Change Device Name (can only 4 bytes...dunno why) and advertise every 10s (this is the max value) instead of 5s (ADVERTISE_INTERVAL_IN_MS)
                         20191129 - Improve battery life by disabling advertising for 60s and advertise only for 3s
*
*/
...
void manageAdvertisingCallBack(void)
{
    /* Note that the manageAvertisingCallBack() executes in interrupt context, so it is safer to do
     * heavy-weight sensor polling from the main thread. */
    triggerManageAvertising = true;
}
...
// 20191130 - Fonction de lecture du voltage controlé avec le générateur de labo
// Lecture OK de 1.8 à 3.3 v
float read_bat_volt(void){
    NRF_ADC->POWER       = 1;
    wait_ms(5);
    NRF_ADC->EVENTS_END  = 0;
    NRF_ADC->TASKS_STOP  = 1;
    NRF_ADC->ENABLE;     // save register value
    NRF_ADC->CONFIG;     // save register value
    NRF_ADC->RESULT;     // save register value
    NRF_ADC->ENABLE = ADC_ENABLE_ENABLE_Enabled;
    NRF_ADC->CONFIG = (ADC_CONFIG_RES_10bit << ADC_CONFIG_RES_Pos) |
                      (ADC_CONFIG_INPSEL_SupplyOneThirdPrescaling << ADC_CONFIG_INPSEL_Pos) |
                      (ADC_CONFIG_REFSEL_VBG << ADC_CONFIG_REFSEL_Pos) |
    ...
    uint16_t wrk = NRF_ADC->RESULT;          // 10 bit result
    NRF_ADC->EVENTS_END  = 0;
    NRF_ADC->TASKS_STOP = 1;
    NRF_ADC->CONFIG      = 0x18;
   ...
   * BLE object is used in the main loop below. */
   ...
   while (true) {
       ...
       if (triggerSensorPolling) {
           ...
           #if DEBUG_MODE
              _device.printf("Battery voltage : %6.2f \r\n", _battery);
           #endif
           ...
           /* trigger turn off advertisement after X seconds */
           _manageAdvertising.attach(manageAdvertisingCallBack, STOP_ADV_INTERVAL_IN_S); 
       }
       
       if (triggerManageAvertising) {
           //Stop advertising
           ble.gap().stopAdvertising();
           _manageAdvertising.detach();
           triggerManageAvertising = false;
       }
