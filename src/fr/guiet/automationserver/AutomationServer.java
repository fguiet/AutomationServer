...
/***
* 
* Classe principale de gestion de l'IoT de la maison
* Creation : 201602
*
*/
public class AutomationServer implements Daemon {
	...
	private WaterHeater _waterHeater = null; //service de gestion du chauffe-eau
    ...
    public void init(DaemonContext daemonContext) throws DaemonInitException, Exception {
        ...
        _mainThread = new Thread() {
            ...
            public synchronized void start() {
                ...
                super.start(); //Démarrage du Thread principal (Appel de la méthode run)
            }
            ...
            public void run() {    
				try {
					...
					//Creation du serveur de reception des messages de la console web
				...
				//	_logger.error("Une erreur est apparue dans le serveur Automation...",ioe);
				//}
				catch(Exception e) {
	...
	private void CreateCheckThreadActivityTask() {
		...
		TimerTask checkThreadActivityTask  = new TimerTask() {        
			...
			public void run()
			{
				if (_roomServiceThread.isAlive()) {
					_logger.info("Thread RoomService Alive");
				}
				else {
				...
				if (_teleInfoServiceThread.isAlive()) {
					_logger.info("Thread TeleInfoService Alive");
				}
				else {
				...
				if (_waterHeaterServiceThread.isAlive()) {
					_logger.info("Thread WaterHeaterService Alive");
				}
				else {
    ...
    public void stop() throws Exception {
        ...
        try{
            _mainThread.join(5000); //Attend la fin de l'exécution du Thread principal (5 secondes)
