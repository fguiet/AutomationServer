...
public class XBeeService implements PacketListener {
		...
		if (response != null) {
			try {
	            ...
	            if (response.getApiId() == ApiId.RX_64_RESPONSE) {
					...
					//_logger.info("Messsage recu"+sb.toString());
					xbeeMessage = new XBeeMessage(rx.getRemoteAddress());
					xbeeMessage.setMessageReceived(sb.toString());
					
	             }
			} catch(Exception ex) {
				xbeeMessage = null;
	        	_logger.error("Erreur dans le traitement des trames reçues par le XBee",ex);
	        }				
		}
	...
	public void SendAsynchronousMessage(XBeeAddress64 sensorAddress, String message) {
		...
		try {
			...
			//_logger.info("Envoi du message : "+message+" au module : "+sensorAddress.toString());
			//RxResponse64 rx = (RxResponse64) _xbee.sendSynchronous(request, 1000);
			//TxStatusResponse response = (TxStatusResponse) _xbee.sendSynchronous(request, 1000);
		}
		/*catch (XBeeTimeoutException te) {
			_logger.error("Timeout lors de l'envoi du message. Le XBee distant est allumé?", te);
		}
		catch(XBeeException e) {
        	_logger.error("Impossible d'ouvrir une connection avec le XBee sur le port : " + DEFAULT_COM_PORT, e);
        }*/
		...
		TxStatusResponse response = null;
		
		try {
			TxRequest64 request = new TxRequest64(_sensorAddress, "GETSENSORINFO");
											
			while(retryCpt <= 3 && !sentOk) {
															
				//_logger.info("Debut envoie message synchrone");
				// send the packet and wait up to 1 seconds for the transmit status reply (you should really catch a timeout exception)
				_logger.info("Envoi du message : "+xbeeMessage.getMessageToSendString()+" au module : "+xbeeMessage.getRemoteAddress().toString()+ " (Tentative numero : "+retryCpt+")");
				response = (TxStatusResponse)_xbee.sendSynchronous(request, 1000);
				...
				if (response.isSuccess()) {
					...
					_logger.warn("Message envoyé à un module mais ACK non recu (Tentative numero : "+retryCpt+")");
					retryCpt++;	
				}							
			}
		}
			...
			_logger.error("Erreur lors de l'envoi du message. Le XBee distant est allumé?", e);
		}
