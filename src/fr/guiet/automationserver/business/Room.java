...
public class Room {
	...
	*/
	public String NextChangeDefaultTemp() {
		...
		while (!found) {
			...
			if (ts != null) {
				...
				String hour = String.format("%02d:%02d", calendarHourBegin.get(Calendar.HOUR_OF_DAY), calendarHourBegin.get(Calendar.MINUTE));																
				result = GetDayName(dayOfWeek) + " à " + hour + " (" +  String.format("%.2f", ts.getDefaultTempNeeded()) + "°C)" ;
				break;
			}		
			...
			if (dayOfWeek==dayOfWeekTemp) {
				_logger.warn("Impossible de déterminer le prochain changement de température par défaut pour les pièces : "+_name);
				result = "NA à NA (NA °C)" ;
				found = true;
				break;
			}
	...
	public synchronized void SetWantedTemp(Float wantedTemp) {
		_logger.info("Modification de la température désirée pour la pièce : "+_name+". Ancienne valeur : "+_userWantedTemp+". Nouvelle valeur : "+wantedTemp);
		_userWantedTemp = wantedTemp;
	}
	
	public boolean AtLeastOneHeaterOn() {
		for(Heater h : _heaterList) {
			if (h.isOn()) return true;
		}
	...
	public Float ComputeWantedTemp() {
		...
		//Probleme lors de la recuperation de la temperature par defaut ou utilisateur
		...
		//la temperature par defaut a changer, on reinitialise la temperature voulu par l'utilisateur
		if (currentDefaultTemp.compareTo(_lastDefaultTemp)!=0) {
			_logger.info(currentDefaultTemp +", "+_lastDefaultTemp+" Réinitialisation de la température désirée par l'utilisateur car il y a un changement de la température par défaut pour la pièce : "+_name);
			_userWantedTemp = currentDefaultTemp;
			_lastDefaultTemp = currentDefaultTemp;
		}
	...
	private Float GetDefaultTempByDayAndTime() {
		...
		if (defaultTemp == null) {
			_logger.warn("Impossible de déterminer la température par défaut pour les pièces : "+_name+ ".Le radiateur ne va pas etre controler correctement...");
		}
	...
	*/
	//private void ManageHeaters() {
		...
		/*if (_sensor.HasTimeoutOccured()) {
			_actualTemp = _sensor.getActualTemp();
			_actualHumidity = _sensor.getActualHumidity();
		}
		else {
			_actualTemp = null;
			_actualHumidity = null;
	...
	private Room(RoomDto dto) {
		...
		//_logger.info("Capteur : " + sensorDto.sensorId + ", address : " + sensorDto.sensorAddress);
		...
		_userWantedTemp = _lastDefaultTemp; //temperature par defaut et temperature utilisateur identique au demarrage
	}
