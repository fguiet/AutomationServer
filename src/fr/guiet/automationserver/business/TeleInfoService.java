...
public class TeleInfoService implements Runnable {
		...
		_sdl = new SerialDataListener() {
			...
			public void dataReceived(SerialDataEvent event) {
				...
				for (int i = 0; i < data.length; i++) {				  
					...
					//et le premier caractere de la trame doit avoir ete verifie
					if (receivedChar == 0x03 && _beginTrameDetected && !_checkFirstChar) 
					{
						_endTrameDetected = true;
					}

					//Si le debut de la trame a ete detecte, on enregistre...on ne 
					//veut pas le caractere de fin evidement ainsi que les caracteres qui ont arrive
					//apres avec le buffer
					if (_beginTrameDetected && !_endTrameDetected) {												

						//Verification du premier carac apres reception du carac de debut
						...
						if (_checkFirstChar) {
							...
							if (receivedChar != 0x0A) { //Line Feed
								...
								//System.out.println("int char : "+ Character.getNumericValue(receivedChar));
							}								
						}
		...
		try {
			...
			while(!_trameFullyReceived) 
			{
				//_logger.info("Buffer Has Data : "+serial.read());
				 //System.out.println("Buffer Has Data : "+serial.availableBytes());
				try {
					...
					//201603 - Hack pour le lancement du service au demarrage...le serial port n'est pas correctement ouvert la première fois??
					if (diffMinutes >=1) {
						_logger.warn("Timeout dans la réception d'une trame, relance d'une écoute sur le serial port...");
						return null;
					}
				}
	...
	{		
			...
			//groupe commence par un line feed \n et termine par un carriage return \r
			String[] groupes = trame.split("\\r");
			
			if (groupes.length != VALID_GROUPES_NUMBER) {
				//_logger.warn("Reception d'une trame invalide : DEBUT"+trame+"FIN. La trame n'a pas ete prise en compte");				
				return null;
			}
			...
			for(String g : groupes) {
				...
				if (Checksum(etiquette, valeur, checksum)) {				
					...
					//_logger.error("Checksum invalide pour l'etiquette : "+etiquette+", valeur : "+valeur);
				}
			}
	...
	private boolean Checksum(String etiquette, String valeur, String checksum) 
	{
		...
		if (sum == checksumChar) return true ;	
